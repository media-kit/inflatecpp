...
Unless otherwise noted in a file's first 5 lines, all source code published on
http://create.stephan-brumme.com and its sub-pages is licensed similar to the
zlib license: This software is provided 'as-is', without any express or implied
...
original software. If you use this software in a product, an acknowledgment in
the product documentation would be appreciated but is not required. Altered
...
constexpr auto kLittleEdian = 1234;
constexpr auto kBigEdian = 4321;
constexpr unsigned int kCrc32Lookup[4][256] = {
    {
...
unsigned int crc32_4bytes(const void* data, unsigned int length,
                          unsigned int previousCrc32) {
  unsigned int crc = ~previousCrc32;
  const unsigned int* current = (const unsigned int*)data;

  while (length >= 4) {
    unsigned int one = *current++ ^ crc;
    crc = kCrc32Lookup[0][(one >> 24) & 0xFF] ^
          kCrc32Lookup[1][(one >> 16) & 0xFF] ^
